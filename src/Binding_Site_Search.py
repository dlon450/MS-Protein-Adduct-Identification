...
def search(bound_file_path, unbound_file_path, compounds_file_path, tolerance=config.tolerance, \
    protein=config.protein, peak_height=config.peak_height, plot_peak_graph=False):
    '''
    ...
    other_compounds_df = compounds[compounds.Name != protein] # extract all compounds but Ubiq
    ...
    binding_sites_df = match_peaks(peaks, binding_df, np.float(tolerance))
